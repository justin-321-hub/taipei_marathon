 ...
 * ★ 新增：將文字中的網址轉換為可點擊的連結
 * @param {string} text - 原始文字
 * @returns {string} - 包含 HTML 連結的文字
 */
function linkify(text) {
  // 先進行 HTML 轉義，防止 XSS
  let escapedText = escapeHtml(text);
  
  // 完整的網址正則表達式，支援各種協議和域名格式
...
function render() {
  ...
  for (const m of messages) {
    ...
    // ★ 修改：使用 innerHTML 並將文字轉換為包含連結的 HTML
    // 只對機器人訊息啟用連結轉換，使用者訊息保持純文字以避免安全問題
    if (isUser) {
      // 使用者訊息：保持純文字但轉義 HTML
